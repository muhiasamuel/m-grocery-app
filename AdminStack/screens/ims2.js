...
export default class Ims2 extends Component {
  _getHeaderLoader = () => (
    <ActivityIndicator style={styles.Done} size='large' color={'#0580FF'}/>
  );
  ...
  imagesCallback = (callback) => {
    ...
    callback.then(async (photos) => {
      ...
      navigation.navigate('editProducts', {photos: cPhotos});
      console.log(cPhotos);
  ...
  _renderDoneButton = (count, onSubmit) => {
    ...
    return <TouchableOpacity title={'Done'} onPress={onSubmit}>
      <Text onPress={onSubmit} style={styles.Done}>Done</Text>
    </TouchableOpacity>
  }
